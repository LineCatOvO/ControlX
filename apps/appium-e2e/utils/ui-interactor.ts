...
class UIInteractor {
    ...
    async checkUIElements(): Promise<UIElements> {
    ...
    async verifyServiceStatus(expectedStatus: "running" | "stopped"): Promise<boolean> {
        ...
        try {
            ...
            const checkMethods = [
                {
                    ...
                    check: (output: string) => output.includes(packageName)
                },
                {
                    name: "dumpsys package (check overlay)",
                    command: `adb -s ${deviceId} shell dumpsys package ${packageName}`,
                    check: (output: string) => output.includes("android.permission.SYSTEM_ALERT_WINDOW") && (output.includes("granted=true") || output.includes("granted") || output.includes("allowed=true"))
                },
                {
                    name: "appops check",
                    command: `adb -s ${deviceId} shell appops get ${packageName} SYSTEM_ALERT_WINDOW`,
                    check: (output: string) => output.includes("allow") || output.includes("granted")
                },
                {
            ...
            for (const method of checkMethods) {
                try {
                    ...
                    const output = execSync(method.command, { stdio: "pipe", encoding: "utf8" });
            ...
            try {
                ...
                const settingsOutput = execSync(
                    `adb -s ${deviceId} shell settings list secure`,
