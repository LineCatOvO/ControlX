...
import com.linecat.wmmtcontroller.debug.DebugModeManager;
...
public final class PlatformAdaptationLayer {
    ...
    /**
     * 检查是否在debug模式下且启用了权限绕过
     */
    private boolean isInDebugModeWithBypass() {
        try {
            DebugModeManager debugManager = DebugModeManager.getInstance(context);
            return debugManager.shouldBypassOverlayPermission();
        } catch (Exception e) {
            Log.w(TAG, "Failed to check debug mode: " + e.getMessage());
            return false;
        }
    }
    ...
    public void startOverlay() {
        ...
        if (overlayMode == OverlayMode.SYSTEM_OVERLAY) {
            // 检查是否在debug模式下绕过权限检查
            boolean bypassPermission = isInDebugModeWithBypass();
            if (!bypassPermission && !android.provider.Settings.canDrawOverlays(context)) {
