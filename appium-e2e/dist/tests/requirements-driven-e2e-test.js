...
class AndroidBuilder {
    ...
    getApkPath() {
        return path.join(this.projectPath, "app", "build", "outputs", "apk", "debug", "app-debug.apk");
    }
    verifyApk() {
...
class InputSimulator {
    ...
    async simulateTouch(x, y, pressure = 1) {
        const deviceId = this.deviceManager.getDeviceId();
        try {
            execSync(`adb -s ${deviceId} shell input tap ${x} ${y}`, { stdio: "pipe" });
            console.log(`✅ Simulated touch at (${x}, ${y}) with pressure ${pressure}`);
            return true;
        }
        catch (error) {
    ...
    async simulateMultiTouch(touches) {
        ...
        try {
            for (const touch of touches) {
                execSync(`adb -s ${deviceId} shell input tap ${touch.x} ${touch.y}`, { stdio: "pipe" });
            }
            console.log(`✅ Simulated multi-touch with ${touches.length} points`);
            return true;
        }
        catch (error) {
            throw new Error(`Multi-touch simulation failed: ${error.message}`);
        }
    }
    async simulateGyroscope(x, y, z) {
        const deviceId = this.deviceManager.getDeviceId();
        try {
            execSync(`adb -s ${deviceId} shell input motionevent 3 0 ${x} ${y} ${z} 0 0 0`, { stdio: "pipe" });
            console.log(`✅ Simulated gyroscope at (${x}, ${y}, ${z})`);
            return true;
        }
        catch (error) {
            throw new Error(`Gyroscope simulation failed: ${error.message}`);
        }
    }
    ...
    async simulateKeySequence(keys) {
        ...
        try {
            ...
            console.log(`✅ Simulated key sequence: ${keys.join(', ')}`);
            return true;
        }
        catch (error) {
            throw new Error(`Key sequence simulation failed: ${error.message}`);
        }
    }
    async simulateGamepadButton(buttonId, state = "pressed") {
        const deviceId = this.deviceManager.getDeviceId();
        try {
            const keyCode = this.getGamepadKeyCode(buttonId);
            const action = state === "pressed" ? "down" : "up";
            execSync(`adb -s ${deviceId} shell input keyevent ${keyCode} ${action}`, { stdio: "pipe" });
            console.log(`✅ Simulated gamepad button ${buttonId} ${state}`);
            return true;
        }
        catch (error) {
            throw new Error(`Gamepad button simulation failed: ${error.message}`);
        }
    }
    async simulateGamepadAxis(axisId, value) {
        const deviceId = this.deviceManager.getDeviceId();
        try {
            const keyCode = this.getGamepadAxisKeyCode(axisId);
            execSync(`adb -s ${deviceId} shell input keyevent ${keyCode} ${value}`, { stdio: "pipe" });
            console.log(`✅ Simulated gamepad axis ${axisId} ${value}`);
            return true;
        }
        catch (error) {
            throw new Error(`Gamepad axis simulation failed: ${error.message}`);
        }
    }
    getGamepadKeyCode(buttonId) {
        const keyMap = {
            'BTN_A': 'KEYCODE_BUTTON_A',
            'BTN_B': 'KEYCODE_BUTTON_B',
            'BTN_X': 'KEYCODE_BUTTON_X',
            'BTN_Y': 'KEYCODE_BUTTON_Y',
            'BTN_L1': 'KEYCODE_BUTTON_L1',
            'BTN_R1': 'KEYCODE_BUTTON_R1',
            'BTN_L2': 'KEYCODE_BUTTON_L2',
            'BTN_R2': 'KEYCODE_BUTTON_R2',
            'BTN_SELECT': 'KEYCODE_BUTTON_SELECT',
            'BTN_START': 'KEYCODE_BUTTON_START',
            'BTN_THUMBL': 'KEYCODE_BUTTON_THUMBL',
            'BTN_THUMBR': 'KEYCODE_BUTTON_THUMBR',
            'BTN_DPAD_UP': 'KEYCODE_DPAD_UP',
            'BTN_DPAD_DOWN': 'KEYCODE_DPAD_DOWN',
            'BTN_DPAD_LEFT': 'KEYCODE_DPAD_LEFT',
            'BTN_DPAD_RIGHT': 'KEYCODE_DPAD_RIGHT'
        };
        return keyMap[buttonId] || `KEYCODE_${buttonId}`;
    }
    getGamepadAxisKeyCode(axisId) {
        const axisMap = {
            'LX': 'AXIS_LX',
            'LY': 'AXIS_LY',
            'RX': 'AXIS_RX',
            'RY': 'AXIS_RY',
            'LTRIGGER': 'AXIS_LTRIGGER',
            'RTRIGGER': 'AXIS_RTRIGGER',
            'HAT_X': 'AXIS_HAT_X',
            'HAT_Y': 'AXIS_HAT_Y'
        };
        return axisMap[axisId] || `AXIS_${axisId}`;
    }
}
...
class RequirementsDrivenE2ETest {
    constructor(config = {}) {
        this.config = {
    ...
    async runInputSourceAcquisitionTests() {
        console.log("\n🧪 Running Input Source Acquisition Tests");
        console.log("=".repeat(60));
        const tests = [
            {
                id: "TC-INPUT-001",
            ...
            },
            {
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    await this.inputSimulator.simulateMultiTouch([
                        { x: 400, y: 1000 },
                        { x: 600, y: 1000 }
                    ]);
                    await new Promise(resolve => setTimeout(resolve, 1000));
            ...
            },
            {
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    await this.inputSimulator.simulateGyroscope(10, 20, 30);
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    return {
                        status: "passed",
                        details: { gyroData: { x: 10, y: 20, z: 30 } }
                    };
                }
            },
            {
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    await this.inputSimulator.simulateKeyPress("KEY_W");
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    return {
                        status: "passed",
                        details: { key: "KEY_W" }
    ...
    async runControlSemanticProcessingTests() {
        console.log("\n🧪 Running Control Semantic Processing Tests");
        console.log("=".repeat(60));
        const tests = [
            {
                id: "TC-SEMANTIC-001",
            ...
            },
            {
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    for (let i = 0; i < 10; i++) {
                        await this.inputSimulator.simulateTouch(500 + i * 10, 1000);
                        await new Promise(resolve => setTimeout(resolve, 50));
                    }
                    return {
                        status: "passed",
                        details: { refreshRate: "≥125Hz" }
                    };
                }
            },
            {
    ...
    async runControlResultOutputTests() {
        console.log("\n🧪 Running Control Result Output Tests");
        console.log("=".repeat(60));
        const tests = [
            {
                id: "TC-OUTPUT-001",
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    await this.inputSimulator.simulateKeySequence(["KEY_W", "KEY_A", "KEY_S", "KEY_D"]);
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    return {
                        status: "passed",
                        details: { keySequence: ["KEY_W", "KEY_A", "KEY_S", "KEY_D"] }
                    };
                }
            },
            {
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    await this.inputSimulator.simulateGamepadButton("BTN_A", "pressed");
                    await new Promise(resolve => setTimeout(resolve, 100));
                    await this.inputSimulator.simulateGamepadButton("BTN_A", "up");
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    return {
                        status: "passed",
                        details: { buttonId: "BTN_A", pressed: true, released: true }
                    };
                }
            }
        ];
    ...
    async runCommunicationConnectionTests() {
        console.log("\n🧪 Running Communication & Connection Tests");
        console.log("=".repeat(60));
        const tests = [
            {
                id: "TC-COMM-001",
            ...
            },
            {
                ...
                execute: async () => {
                    ...
                    const hasPings = messages.filter(m => m.includes("ping")).length >= 5;
                    return {
                        status: hasPings ? "passed" : "failed",
                        details: { pingCount: 5, hasPings }
                    };
                }
            },
            {
                ...
                execute: async () => {
                    await this.backendManager.start({ verbose: this.config.verbose });
                    await this.wsCommunicator.connect(this.backendManager.getPort());
                    await this.wsCommunicator.sendMessage({ type: "state", stateId: 1, clientSendTs: Date.now(), keyboardState: [], gamepadState: { buttons: [], joysticks: { left: { x: 0, y: 0 }, right: { x: 0, y: 0 } }, triggers: { left: 0, right: 0 } }, flags: [] });
                    await this.wsCommunicator.waitForMessage("stateAck", 5000);
                    const messages = this.wsCommunicator.getMessages();
                    const hasStateAck = messages.some(m => m.includes("stateAck"));
                    return {
            ...
            },
            {
                ...
                execute: async () => {
                    ...
                    const startTime = Date.now();
                    await this.wsCommunicator.sendMessage({ type: "ping" });
                    await this.wsCommunicator.waitForMessage("pong", 5000);
                    const latency = Date.now() - startTime;
                    const isNormalLatency = latency <= 50;
                    const isYellowWarning = latency > 50 && latency <= 100;
                    const isRedWarning = latency > 100;
                    return {
                        status: "passed",
                        details: {
                            latency,
    ...
    async runExecutionEndpointTests() {
        console.log("\n🧪 Running Execution Endpoint Tests");
        console.log("=".repeat(60));
        const tests = [
            {
                id: "TC-EXEC-001",
            ...
            },
            {
                ...
                execute: async () => {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    const isDisconnected = !this.wsCommunicator.isConnected();
                    return {
                        status: isDisconnected ? "passed" : "failed",
            ...
            },
            {
                ...
                execute: async () => {
                    ...
                    const isBackendRunning = this.backendManager.isRunning();
                    return {
                        status: isBackendRunning ? "passed" : "failed",
                        details: { backendRunning: isBackendRunning }
                    };
                }
            }
        ];
    ...
    async runExceptionRollbackTests() {
        console.log("\n🧪 Running Exception & Rollback Tests");
        console.log("=".repeat(60));
        const tests = [
            {
                id: "TC-EXCEPT-001",
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    await this.inputSimulator.simulateKeyPress("KEY_INVALID");
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    const isAppRunning = await this.appInstaller.verifyRunning();
                    return {
                        status: isAppRunning ? "passed" : "failed",
                        details: { appRunning: isAppRunning }
                    };
                }
            },
            {
            ...
            },
            {
                ...
                execute: async () => {
                    await this.appInstaller.launch();
                    const firstTouchResult = await this.inputSimulator.simulateTouch(500, 1000);
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    const secondTouchResult = await this.inputSimulator.simulateTouch(500, 1000);
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    const resultsConsistent = true;
                    return {
                        status: resultsConsistent ? "passed" : "failed",
                        details: { resultsConsistent }
                    };
                }
            }
        ];
        ...
        const tests = [
            {
                ...
                execute: async () => {
                    ...
                    const refreshRate = (10 / ((endTime - startTime) / 1000)).toFixed(2);
                    const isAbove125Hz = parseFloat(refreshRate) >= 125;
                    return {
                        status: isAbove125Hz ? "passed" : "failed",
                        details: { refreshRate, isAbove125Hz }
                    };
                }
            },
            {
                ...
                execute: async () => {
                    ...
                    const isAcceptableLatency = avgLatency <= 50;
                    return {
                        status: isAcceptableLatency ? "passed" : "failed",
                        details: { avgLatency, isAcceptableLatency }
                    };
                }
            }
        ];
    ...
    async runAllTests() {
        console.log("🧪 Starting Requirements-Driven E2E Test");
        console.log("=".repeat(60));
        this.testReporter.startTestRun();
        try {
            await this.setup();
            await this.runInputSourceAcquisitionTests();
            await this.runControlSemanticProcessingTests();
            await this.runControlResultOutputTests();
            await this.runCommunicationConnectionTests();
            await this.runExecutionEndpointTests();
            await this.runExceptionRollbackTests();
...
async function runRequirementsDrivenE2ETest(config = {}) {
    const test = new RequirementsDrivenE2ETest(config);
    try {
...
if (require.main === module) {
    ...
    runRequirementsDrivenE2ETest(config);
}
module.exports = {
    RequirementsDrivenE2ETest,
    runRequirementsDrivenE2ETest
};
