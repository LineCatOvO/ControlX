...
async function testServiceWithoutOverlayPermission() {
    ...
    try {
        ...
        try {
            ...
            console.log(
                "   Current permission status check result:",
                permissionCheck.trim() || "No specific permission data"
            );
        } catch (error) {
            console.log(
                "   Unable to check detailed permission status, proceeding with test"
            );
        }
        ...
        try {
            ...
            console.log(
                "⚠️  Service start command may have failed:",
                error.message
            );
        }
        ...
        try {
            ...
            if (serviceDump.includes("InputRuntimeService")) {
                ...
                console.log(
                    "   Service dump snippet:",
                    serviceDump.substring(0, 200) + "..."
                );
            } else {
                console.log(
                    "⚠️  InputRuntimeService not found in service dump"
                );
            }
        } catch (error) {
        ...
        try {
            ...
            if (logOutput) {
                ...
                if (
                    logOutput.includes(
                        "Skipping automatic overlay permission check"
                    )
                ) {
                    console.log(
                        "✅ Found log indicating permission check was skipped"
                    );
                }

                if (
                    logOutput.includes(
                        "ACTIVITY_PANEL mode without overlay permission"
                    )
                ) {
                    console.log(
                        "✅ Found log indicating ACTIVITY_PANEL mode is being used"
                    );
                }
            } else {
