...
**✅ Appium 模拟真实用户交互** - 所有测试通过 Appium 点击、滑动等操作模拟用户行为

**⚠️ WebSocket 仅用于验证** - WebSocket 只监听后端收到的输入，不主动发送输入数据
...
| 用途 | 是否符合 | 说明 |
|------|----------|------|
| 监听后端收到的输入 | ✅ 正确 | 验证 App→Server 通信正常 |
| 发送 Ping 测网络 RTT | ✅ 正确 | 网络层延迟基准 |
| 接收服务器广播消息 | ✅ 正确 | 验证服务器推送能力 |
| **主动发送输入数据** | ❌ **错误** | **跳过了 App 逻辑，不是 E2E 测试** |
